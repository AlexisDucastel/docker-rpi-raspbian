...
[mkimage-raspbian.sh](https://github.com/alexisducastel/docker-rpi-raspbian/blob/master/mkimage-raspbian.sh) is used to build and configure the chroot. This script **heavily** borrows from docker's [mkimage.sh](https://github.com/docker/docker/blob/master/contrib/mkimage.sh) script.
...
```bash
$ docker run --name raspbian -it aducastel/rpi-raspbian:latest /bin/bash
```
